...
local function process_inputs(pos)
	...
	if not player then
		...
		core.get_meta(pos):set_string("infotext","Digilines Game Controller Ready\n(right-click to use)")
		players_on_controller[hash] = nil
		return
	end
...
local function trap_player(pos,player)
	...
	if oldname and core.get_player_by_name(oldname) then
			core.chat_send_player(newname,
		...
		core.chat_send_player(newname,"You are now using a digilines game controller. Right-click the controller again to be released.")
		local meta = core.get_meta(pos)
	...
	digiline = {
		...
		effector = {
			action = function(pos,node,channel,msg)
				local setchannel = core.get_meta(pos):get_string("channel")
